...
- `lambda/` — endpoint público en **AWS Lambda** `candyads-lead` (eu-west-1, Function URL, auth NONE; CORS en el código, solo `https://candyads.es`). Rutas `/challenge` (reto ALTCHA firmado), `/lead`, `/confirmar` (confirmación de venta, sin ALTCHA: el riesgo lo acota que `ref` es un uuid v4 impredecible) y `/contacto` (formulario de contacto de la landing, ver abajo). Node: `npm test` (23 tests), `npm run build` -> `dist/index.mjs` y zip para subir a mano en la consola de Lambda. Falla cerrado si falta configuración.
...
- `supabase/migrations/` — proyecto Supabase `candyads-leads` (eu-central-1). Solo contadores (`leads_count`), email de destino por anunciante (`anunciantes_destino`), retos ALTCHA usados, estado de conversión de cada lead y admins del panel. Lambda solo llama a funciones RPC (`destino_de`, `reservar_lead_ref`, `confirmar_lead_ref`, `fallar_lead_ref`, `consumir_reto`, `marcar_conversion`; `reservar_lead`/`liberar_lead` son las antiguas, ya sin uso) con la secret key; un anónimo recibe 401 en todo. El formulario de contacto de la landing (`/contacto`) NO usa Supabase: es solo Lambda → SES, sin contador ni trazabilidad (no lo pidió la revisión legal, y no es un sistema multi-anunciante).
...
Estado: Fases 1, 2a y 2b hechas y probadas en producción. SES aprobado en producción el 2026-09-22 (ver Pendiente conocido) — **técnicamente ya se podría enviar a la dirección real de José**, pero el destino de `lacasa-piloto` sigue siendo un email de PRUEBAS a propósito: la revisión legal (Fase 2 más abajo) exige tener antes `razon_social`/`nif_cif`/`email_privacidad` reales de La Casa Agency; no cambiar el destino en Supabase hasta tenerlos. Pendiente: pasar Supabase a Pro, panel interno con Supabase Auth (2c, parte 2; el generador de QR ya está hecho), datos legales de La Casa y revisión de un abogado sobre las plantillas de `legal/`. La política de privacidad ya incluye la sección 8 (leads, encargado del tratamiento); el borrador de consulta jurídica general vive fuera del repo (público), en `Desktop\candyads-abogado`.
