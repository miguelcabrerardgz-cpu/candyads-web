...
Fase 3 (hecha, publicada): Formspree sustituido por la ruta `/contacto` de la misma Lambda + SES (ver arriba); ya no hay ninguna mención a Formspree en el repo. El formulario de la landing (`index.html`) llama a `ENDPOINT_CONTACTO + '/contacto'` igual que `lead.js` llama a `/lead`. Pendiente: subir el zip nuevo a la Lambda y añadir la variable `CONTACT_TO` en su consola si no estaba ya (ver estado más abajo); probar un envío real.
